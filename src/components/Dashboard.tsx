      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-200/60">
          <div className="space-y-2">
            {org.logo_url ? (
              <img src={org.logo_url} alt={org.name} className="h-14 object-contain mb-4" />
            ) : (
            ...
            <div>
              ...
              <p className="text-slate-500 text-base font-medium mt-2">You have <span className="text-slate-900 font-semibold">{documents.filter(d => d.status === 'pending').length} pending</span> documents to sign.</p>
            </div>
          </div>
          
          <button 
            onClick={handleLogout}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-900 transition-colors border border-slate-200/60 shadow-sm"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition-shadow">
             ...
             <p className="text-4xl font-bold tracking-tight text-slate-900">{documents.length}</p>
             <p className="text-base font-semibold text-slate-500 mt-1">Total Assignments</p>
          </div>
          <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition-shadow">
             ...
             <p className="text-4xl font-bold tracking-tight text-slate-900">{documents.filter(d => d.status === 'pending').length}</p>
             <p className="text-base font-semibold text-slate-500 mt-1">Pending Actions</p>
          </div>
          <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition-shadow">
             ...
             <p className="text-4xl font-bold tracking-tight text-slate-900">{documents.filter(d => d.status === 'signed').length}</p>
             <p className="text-base font-semibold text-slate-500 mt-1">Completed Items</p>
          </div>
        </div>
              ...
              documents.map((doc) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-slate-900 text-base group-hover:text-primary transition-colors">{doc.rams_documents.name}</h3>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {doc.status === 'pending' ? (
                      <Link 
                        href={`/sign/${doc.token}`}
                        className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold text-base shadow-sm hover:bg-primary/90 transition-colors"
                      >
                        Sign
                      ...
                      <div className="flex items-center gap-2">
                        <Link 
                          href={`/sign/${doc.token}`}
                          className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-slate-50 border border-slate-200/60 text-slate-700 font-semibold text-base hover:bg-slate-100 transition-colors shadow-sm"
                        >
                          View
