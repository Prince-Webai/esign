    ...
    <div className="space-y-8 pb-12 animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      <div className="bg-white border border-slate-200/60 rounded-2xl p-6 md:p-8 shadow-sm">
        <div className="flex items-center gap-4 mb-6">
           ...
           <div>
              <h2 className="text-xl font-bold text-slate-900 tracking-tight">Add User</h2>
              <p className="text-slate-500 text-sm">Provision a new account for this portal</p>
           </div>
        </div>
        ...
        <form onSubmit={handleAddUser} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          <div className="space-y-1.5 text-left md:col-span-1">
            <label className="text-[11px] font-medium text-slate-500 pl-1">Name</label>
            <input 
              value={newName} onChange={(e) => setNewName(e.target.value)}
              placeholder="Full Name"
              className="w-full bg-white border border-slate-200/60 rounded-xl px-4 py-2.5 text-sm font-medium text-slate-900 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all placeholder:text-slate-400 shadow-sm"
            />
          </div>
          <div className="space-y-1.5 text-left md:col-span-1">
            <label className="text-[11px] font-medium text-slate-500 pl-1">Email</label>
            <input 
          ...
          <div className="space-y-1.5 text-left md:col-span-1">
            <label className="text-[11px] font-medium text-slate-500 pl-1">Security PIN</label>
            <div className="relative">
               ...
               <input 
                 ...
                 className="w-full bg-white border border-slate-200/60 rounded-xl pl-9 pr-4 py-2.5 text-sm font-medium text-slate-900 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all placeholder:text-slate-400 shadow-sm"
               />
            </div>
          </div>
          <div className="space-y-1.5 text-left md:col-span-1">
            <label className="text-[11px] font-medium text-slate-500 pl-1">Account Role</label>
            <div className="relative group">
              <select 
                value={newRole} 
                onChange={(e) => setNewRole(e.target.value)}
                className="w-full bg-white border border-slate-200/60 rounded-xl px-4 py-2.5 text-sm font-medium text-slate-900 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none appearance-none cursor-pointer shadow-sm"
              >
          ...
          <button 
            disabled={isAdding}
            type="submit"
            className="bg-emerald-600 text-white font-medium h-[42px] rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm hover:bg-emerald-700 text-sm active:scale-95 disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-semibold text-slate-900 flex items-center gap-2 pl-2">
    ...
    <div className="group flex flex-col hover:bg-slate-50/50 transition-all divide-y divide-slate-100">
      <div className="flex items-center justify-between p-4 md:p-6">
        <div className="flex items-center gap-4">
          <div className={cn(
            "w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold shadow-sm border transition-transform group-hover:scale-110 duration-300",
            user.role === 'admin' ? "bg-amber-50 border-amber-100 text-amber-600" : "bg-emerald-50 border-emerald-100 text-emerald-600"
          )}>
            {user.name.charAt(0).toUpperCase()}
          </div>
          <div className="space-y-0.5">
            <p className="font-medium text-slate-900 text-sm leading-tight tracking-tight">{user.name}</p>
